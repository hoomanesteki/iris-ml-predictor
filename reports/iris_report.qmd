...
metrics = pd.read_csv("results/metrics/metrics_summary.csv")
...
```{python}
#| label: dummy-metrics
#| tbl-cap: Dummy Classifier Performance Metrics
dummy_metrics = pd.read_csv("results/metrics/dummy_report_df.csv")
dummy_accuracy = round(dummy_metrics.iloc[0,1] * 100, 2)
```

![Confusion matrix](results/figures/dummy_confusion_matrix.png){#fig-dum_conf width=70%}

We observed nearly `{python} dummy_accuracy`% correct predictions for the three equally balanced classes.
